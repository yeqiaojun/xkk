...
fn global_registry_is_complete_and_initialized_once() {
    ...
    assert!(matches!(
        global_registry(),
        Err(Error::RegistryNotInitialized)
    ));

    init_global_registry().unwrap();
    let registry = global_registry().unwrap();
    assert_eq!(
        registry.response_id(MsgId::PingReq.as_u16()),
        Some(MsgId::PingRsp.as_u16())
    );
    assert_eq!(
        registry.request_id(MsgId::PingRsp.as_u16()),
    ...
    assert!(
        registry
            .new_message(MsgId::PingReq.as_u16())
            .unwrap()
            .downcast_ref::<pb::PingReq>()
            .is_some()
    );
    ...
    assert!(matches!(
        err,
        xkk_protocol::ProtocolError::Registry(Error::RegistryAlreadyInitialized)
    ));
}
